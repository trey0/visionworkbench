...
namespace mosaic {
  ...
  UTMKMLQuadTreeConfig::UTMKMLQuadTreeConfig()
    : m_data( new UTMKMLQuadTreeConfigData() )
  {}

  void UTMKMLQuadTreeConfig::set_longlat_bbox( BBox2 const& bbox ) {
    m_data->m_longlat_bbox = bbox;
  }

  void UTMKMLQuadTreeConfig::set_title( std::string const& title ) {
    m_data->m_title = title;
  }

  void UTMKMLQuadTreeConfig::set_max_lod_pixels( int32 pixels ) {
    m_data->m_max_lod_pixels = pixels;
  }

  void UTMKMLQuadTreeConfig::set_draw_order_offset( int32 offset ) {
    m_data->m_draw_order_offset = offset;
  }

  void UTMKMLQuadTreeConfig::set_metadata( std::string const& data ) {
  ...
  void UTMKMLQuadTreeConfig::configure( QuadTreeGenerator& qtree ) const {
    ...
    qtree.set_metadata_func( boost::bind(&UTMKMLQuadTreeConfigData::metadata_func,m_data,_1,_2) );
    qtree.set_branch_func( boost::bind(&UTMKMLQuadTreeConfigData::branch_func,m_data,_1,_2,_3) );
    qtree.set_tile_resource_func( boost::bind(&UTMKMLQuadTreeConfigData::tile_resource_func,m_data,_1,_2,_3) );
  }
  ...
  std::string UTMKMLQuadTreeConfigData::kml_latlonbox( BBox2 const& longlat_bbox, bool alt ) const {
  ...
  std::string UTMKMLQuadTreeConfigData::kml_network_link( std::string const& name, std::string const& href, BBox2 const& longlat_bbox, int min_lod_pixels ) const {
    std::ostringstream tag;
    tag << std::setprecision(10);
    tag << "  <NetworkLink>\n"
        << "    <name>" << name << "</name>\n"
  ...
  std::string UTMKMLQuadTreeConfigData::kml_ground_overlay( std::string const& href, BBox2 const& region_bbox, BBox2 const& image_bbox, int draw_order, int min_lod_pixels, int max_lod_pixels ) const {
    std::ostringstream tag;
    tag << std::setprecision(10);
    tag << "  <GroundOverlay>\n"
  ...
  BBox2 UTMKMLQuadTreeConfigData::pixels_to_longlat( BBox2i const& image_bbox, Vector2i const& dimensions ) const {
  ...
  void UTMKMLQuadTreeConfigData::metadata_func( QuadTreeGenerator const& qtree, QuadTreeGenerator::TileInfo const& info ) const {
  ...
  std::vector<std::pair<std::string,vw::BBox2i> > UTMKMLQuadTreeConfigData::branch_func( QuadTreeGenerator const& qtree, std::string const& name, BBox2i const& region ) const {
